...
def phase1_compiler(iterations: int = 5000, warmup: int = 500):
    ...
    for num_tables in table_counts:
        ...
        print(
            f"  {'compile':>20s}  avg: {_format_ms(pct['avg'])}  p50: {_format_ms(pct['p50'])}  p95: {_format_ms(pct['p95'])}  p99: {_format_ms(pct['p99'])}"
        )
    ...
    print(
        f"  {'Tables':>8s}  {'p50':>10s}  {'p95':>10s}  {'p99':>10s}  {'max':>10s}  {'SQL len':>8s}"
    )
    for r in results:
        print(
            f"  {r['tables']:>8d}  {_format_ms(r['p50']):>10s}  {_format_ms(r['p95']):>10s}  {_format_ms(r['p99']):>10s}  {_format_ms(r['max']):>10s}  {r['avg_sql_len']:>6d}ch"
        )
...
def phase2_database(dataset: str = "medium"):
    ...
    from app.database.connection import execute_secure_query, get_db_cursor
    ...
    print(
        f"  Seeding: {sizes['customers']:,} customers, {sizes['orders']:,} orders, {sizes['products']:,} products"
    )
                ...
                if existing < count:
                    ...
                    if table == "customers":
                        _seed_table(
                            cursor,
                            table,
                            ["name", "email", "country", "status", "tenant_id"],
                            count,
                            "bench_tenant",
                        )
                    elif table == "orders":
                        _seed_table(
                            cursor,
                            table,
                            ["customer_id", "total_amount", "order_status", "tenant_id"],
                            count,
                            "bench_tenant",
                        )
                    elif table == "products":
                        _seed_table(
                            cursor,
                            table,
                            ["product_name", "price", "stock_count", "category", "tenant_id"],
                            count,
                            "bench_tenant",
                        )
    ...
    for name, blueprint in queries:
        ...
        results.append(
            {"name": name, "compile": compile_pct, "exec": exec_pct, "avg_rows": round(avg_rows)}
        )

        print(f"\n  {name}")
        print(
            f"    {'compile':>12s}  avg: {_format_ms(compile_pct['avg'])}  p50: {_format_ms(compile_pct['p50'])}  p95: {_format_ms(compile_pct['p95'])}  p99: {_format_ms(compile_pct['p99'])}"
        )
        print(
            f"    {'db exec':>12s}  avg: {_format_ms(exec_pct['avg'])}  p50: {_format_ms(exec_pct['p50'])}  p95: {_format_ms(exec_pct['p95'])}  p99: {_format_ms(exec_pct['p99'])}"
        )
        print(f"    {'rows':>12s}  avg: {avg_rows:.0f}")

    _print_header("PHASE 2 SUMMARY")
    print(
        f"  {'Query':<20s}  {'Compile p50':>12s}  {'Exec p50':>12s}  {'Exec p99':>12s}  {'Rows':>8s}"
    )
    for r in results:
        print(
            f"  {r['name']:<20s}  {_format_ms(r['compile']['p50']):>12s}  {_format_ms(r['exec']['p50']):>12s}  {_format_ms(r['exec']['p99']):>12s}  {r['avg_rows']:>8.0f}"
        )
...
def phase3_load(concurrency_levels: list[int] = None, duration: int = 10):
    ...
    from app.compiler.sql_builder import build_secure_query
    from app.database.cache import get_cached_results
    ...
    for users in concurrency_levels:
        ...
        total_pct = (
            _percentiles(total_times)
            if total_times
            else {"min": 0, "p50": 0, "p95": 0, "p99": 0, "max": 0, "avg": 0}
        )
        compile_pct = (
            _percentiles(compile_times)
            if compile_times
            else {"min": 0, "p50": 0, "p95": 0, "p99": 0, "max": 0, "avg": 0}
        )
        cache_pct = (
            _percentiles(cache_times)
            if cache_times
            else {"min": 0, "p50": 0, "p95": 0, "p99": 0, "max": 0, "avg": 0}
        )
        exec_pct = (
            _percentiles(execute_times)
            if execute_times
            else {"min": 0, "p50": 0, "p95": 0, "p99": 0, "max": 0, "avg": 0}
        )
        ...
        print(
            f"  {'total':>20s}  avg: {_format_ms(total_pct['avg'])}  p50: {_format_ms(total_pct['p50'])}  p95: {_format_ms(total_pct['p95'])}  p99: {_format_ms(total_pct['p99'])}"
        )
        print(
            f"  {'compile':>20s}  avg: {_format_ms(compile_pct['avg'])}  p50: {_format_ms(compile_pct['p50'])}  p95: {_format_ms(compile_pct['p95'])}"
        )
        print(
            f"  {'cache':>20s}  avg: {_format_ms(cache_pct['avg'])}  p50: {_format_ms(cache_pct['p50'])}  p95: {_format_ms(cache_pct['p95'])}"
        )
        print(
            f"  {'db execute':>20s}  avg: {_format_ms(exec_pct['avg'])}  p50: {_format_ms(exec_pct['p50'])}  p95: {_format_ms(exec_pct['p95'])}"
        )

    _print_header("PHASE 3 SUMMARY")
    print(
        f"  {'Threads':>8s}  {'Reqs':>8s}  {'Req/s':>10s}  {'Total p50':>12s}  {'Total p99':>12s}  {'Errors':>8s}"
    )
    for r in results:
        print(
            f"  {r['users']:>8d}  {r['count']:>8d}  {r['throughput']:>10.1f}  {_format_ms(r['total']['p50']):>12s}  {_format_ms(r['total']['p99']):>12s}  {r['error_rate']:>6.1f}%"
        )
...
def phase4_failure():
    ...
    results.append(
        {"test": "Empty prompt", "status": "PASS" if passed else "FAIL", "code": resp.status_code}
    )
    ...
    results.append(
        {"test": "Missing prompt", "status": "PASS" if passed else "FAIL", "code": resp.status_code}
    )
    ...
    print(
        f"  {'✓' if passed else '✗'} Metrics endpoint → {resp.status_code} (has Prometheus output: {passed})"
    )
    ...
    print(
        f"  {'✓' if has_req_id else '✗'} X-Request-ID in response: {resp.headers.get('x-request-id', 'MISSING')}"
    )
    ...
    print(
        f"  {'✓' if all_headers else '✗'} Security headers (X-Frame-Options, X-Content-Type-Options, HSTS): {'all present' if all_headers else 'MISSING'}"
    )
    ...
    if resp.status_code == 200:
        ...
        print(
            f"  {'✓' if has_trace else '✗'} Trace spans in response: {len(data.get('trace', {}).get('spans', []))} spans"
        )
        results.append(
            {"test": "Request ID in body", "status": "PASS" if has_request_id else "FAIL"}
        )
        print(
            f"  {'✓' if has_request_id else '✗'} Request ID in response body: {data.get('request_id', 'MISSING')[:12]}..."
        )
    else:
...
Examples:
  ...
  python benchmark.py --phase 2 --dataset large
    ...
    parser.add_argument(
        "--phase", type=int, choices=[1, 2, 3, 4], help="Run a specific phase (default: all)"
    )
    parser.add_argument(
        "--users", type=int, default=50, help="Max concurrent users for phase 3 (default: 50)"
    )
    parser.add_argument(
        "--duration", type=int, default=10, help="Load test duration in seconds (default: 10)"
    )
    parser.add_argument(
        "--dataset",
        choices=["small", "medium", "large"],
        default="medium",
        help="Dataset size for phase 2",
    )
    parser.add_argument(
        "--iterations", type=int, default=5000, help="Iterations for phase 1 (default: 5000)"
    )
