...
def validate_api_key(api_key: str = Security(api_key_header)) -> dict:
    ...
    if not api_key:
        raise HTTPException(
            ...
            detail=(
                f"Authentication credentials missing. Please provide the '{API_KEY_NAME}' header."
            ),
        )

    if api_key not in MOCK_TENANT_REGISTRY:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Invalid or deactivated API key provided."
        )
